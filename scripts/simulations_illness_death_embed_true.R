...
sum_all_comp = function(a, t_type0, t_type1, t_type2, t_type3, mc_cores){
  type_1_comp = -sum(unlist(mclapply(seq_len(nrow(t_type0)), function(i) (log(S_01_comp(a = a,  t = t_type0[i, 1], x = t_type0[i, 2])*S_02_comp(a = a,  t = t_type0[i, 1],
                                                                                                                                         x = t_type0[i, 2]))))))
  type_2_comp = -sum(unlist(mclapply(seq_len(nrow(t_type1)), function(i)(log(as.numeric(
  ...
  type_3_comp = -sum(unlist(mclapply(seq_len(nrow(t_type2)), function(i)(log(as.numeric(
  ...
  type_4_comp = -sum(unlist(mclapply(seq_len(nrow(t_type3)), function(i) (log(f_02_comp(a = a,  t = t_type3[i, 1], x = t_type3[i, 2])*S_01_comp(a = a,  t = t_type3[i, 1], x = t_type3[i, 2]))))))

  type_1_comp + type_2_comp + type_3_comp + type_4_comp
}

sum_all_embed = function(a, t_type0, t_type1, t_type2, t_type3, mc_cores){
  type_1_embed = -sum(unlist(mclapply(seq_len(nrow(t_type0)), function(i) (log(S_01_embed(a = a,  t = t_type0[i, 1],
                                                                                   x = t_type0[i, 2])+S_02_embed(a = a,  t = t_type0[i, 1], x = t_type0[i, 2]))))))
  type_2_embed = -sum(unlist(mclapply(seq_len(nrow(t_type1)), function(i)(log(as.numeric(
    integrate(f01_S12_S02_embed, lower = t_type1[i,1], upper = t_type1[i,2], w = t_type1[i,3],
              a = a, x = t_type1[i, 4])[1]))),mc.cores = mc_cores), use.names = FALSE))
  type_3_embed = -sum(unlist(mclapply(seq_len(nrow(t_type2)), function(i)(log(as.numeric(

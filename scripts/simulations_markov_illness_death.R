...
sum_all = function(a, t_type0, t_type1, t_type2, t_type3, mc_cores){
  type_1 = -sum(unlist(mclapply(seq_len(nrow(t_type0)), function(i) (log(S_01(a = a,  t = t_type0[i, 1])*S_02(a = a,  t = t_type0[i, 1]))))))
  type_2 = -sum(unlist(mclapply(seq_len(nrow(t_type1)), function(i)(log(as.numeric(
    integrate(f01_S12_S02, lower = t_type1[i,1], upper = t_type1[i,2], w = t_type1[i,3],
              a = a)[1]))),mc.cores = mc_cores), use.names = FALSE))
  type_3 = -sum(unlist(mclapply(seq_len(nrow(t_type2)), function(i)(log(as.numeric(
...
sum_all_markov = function(a, t_type0, t_type1, t_type2, t_type3, mc_cores){
  type_1_exp = -sum(unlist(mclapply(seq_len(nrow(t_type0)), function(i) (log(S_01_exp(a = a,  t = t_type0[i, 1])*S_02_exp(a = a,  t = t_type0[i, 1]))))))
  type_2_exp = -sum(unlist(mclapply(seq_len(nrow(t_type1)), function(i)(log(as.numeric(
    integrate(f01_S12_S02_exp, lower = t_type1[i,1], upper = t_type1[i,2], w = t_type1[i,3],
              a = a)[1]))),mc.cores = mc_cores), use.names = FALSE))
  type_3_exp = -sum(unlist(mclapply(seq_len(nrow(t_type2)), function(i)(log(as.numeric(
